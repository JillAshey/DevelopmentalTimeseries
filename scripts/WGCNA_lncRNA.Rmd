...
```{r}
counts <- read.csv("../output/Molecular/lncRNA/lncRNA_filtered_counts.csv")

# Convert to integers
numeric_cols <- sapply(counts, is.numeric)
counts[numeric_cols] <- lapply(counts[numeric_cols], function(x) as.integer(round(x, 0)))
str(counts)
```
...
M10 is a little bit of an outlier but going to keep it for now. 
...
save(sft, file = "~/Desktop/PutnamLab/DevelopmentalTimeseries/sft_lncRNA.RData")
load("~/Desktop/PutnamLab/DevelopmentalTimeseries/sft_lncRNA.RData")
...
# png to output 
png("../output/Molecular/lncRNA/WGCNA/sft.png", 1000, 1000, pointsize=20)
par(mfrow = c(1,2));
cex1 = 0.9;
...
TOM_sign_lncrna = TOMsimilarity(adjacency_sign, TOMType="signed") #Translate adjacency into topological overlap matrix
save(TOM_sign_lncrna, file = "~/Desktop/PutnamLab/DevelopmentalTimeseries/TOM_sign_lncRNA.Rdata")
#load("~/Desktop/PutnamLab/DevelopmentalTimeseries/TOM_sign_lncRNA.Rdata")

dissTOM_sign   = 1-TOM_sign_lncrna
```
...
sizeGrWindow(8,6)
png("../output/Molecular/lncRNA/WGCNA/GeneDendrogram.png", 1000, 1000, pointsize=20)
plotDendroAndColors(geneTree_sign, dynamicColors_sign, "Dynamic Tree Cut - SIGNED",
                    dendroLabels = FALSE, hang = 0.03,
                    addGuide = TRUE, guideHang = 0.05,
                    main = "Gene dendrogram and module colors 'SIGNED'")
...
# Plot the result
sizeGrWindow(7, 6)
png("../output/Molecular/lncRNA/WGCNA/ClusterEigengenes.png", 1000, 1000, pointsize=20)
...
Light yellow module above the cutoff

###  Step 7: Specify the cut line for the dendrogram (module) - Calc MODULE EIGENGENES (mergeMEs)
#### We had 32 modules before merging, and 16 modules after merging
...
sizeGrWindow(7, 6)
png("../output/Molecular/lncRNA/WGCNA/ClusterEigengenes_merged.png", 1000, 1000, pointsize=20)
plot(METree2, main = "Clustering of module eigengenes - SIGNED (dissimilarity calc = MEDiss = 1-cor(MEs))",
     xlab = "", sub = "")
dev.off()
```
...
png("../output/Molecular/lncRNA/WGCNA/ClusterDendrogram_signed.png", 1000, 1000, pointsize=20)
plotDendroAndColors(geneTree_sign, cbind(dynamicColors_sign, mergedColors),
                    c("Dynamic Tree Cut", "Merged dynamic"),
                    dendroLabels = FALSE, hang = 0.03,
                    addGuide = TRUE, guideHang = 0.05)
...
save(MEs, moduleLabels, moduleColors, geneTree_sign, file = "~/Desktop/PutnamLab/DevelopmentalTimeseries/networkConstruction-stepByStep_lncRNA.RData")
# write csv - save the module eigengenes
write.csv(MEs, file = "../output/Molecular/lncRNA/WGCNA/WGCNA_ModuleEigengenes.csv")
table(mergedColors)
```
...
gene_id <- read.csv("../output/Molecular/lncRNA/lncRNA_filtered_counts.csv")[,1]

# Bind gene_id with geneInfo and save as csv 
all_info <- cbind(geneInfo, gene_id)
write.csv(all_info, "../output/Molecular/lncRNA/WGCNA/module_gene_trait_correlations.csv")

# Read in again if needed
all_info <- read.csv("../output/Molecular/lncRNA/WGCNA/module_gene_trait_correlations.csv")
...
pdf(file = "../output/Molecular/lncRNA/WGCNA/Module-trait-relationship-heatmap.pdf", height = 11.5, width = 8)
...
Look at the plot to determine the following groups: maternal, MZT1, MZT2, ZGA1, ZGA2. 

- Maternal - midnightblue, brown, green, purple 
- MZT1 - royalblue, lightyellow, magenta
- MZT2 - blue, cyan, pink
- ZGA1 - red, lightcyan
- ZGA2 - lightgreen, grey60, salmon, black

Make ME boxplots by hpf 
```{r}
MEs <- read.csv("../output/Molecular/lncRNA/WGCNA/WGCNA_ModuleEigengenes.csv")
...
ggsave("../output/Molecular/lncRNA/WGCNA/MEs_boxplot.pdf", plot = MEplot, height = 15, width = 25)
ggsave("../output/Molecular/lncRNA/WGCNA/MEs_boxplot.png", plot = MEplot, height = 15, width = 25)
...
ggsave("../output/Molecular/lncRNA/WGCNA/MEs_lineplot.pdf", plot = MEplot_line, height = 15, width = 25)
ggsave("../output/Molecular/lncRNA/WGCNA/MEs_lineplot.png", plot = MEplot_line, height = 15, width = 25)
```

MZT2 looks more like ZGA1? Discuss w Hollie 
...
ggsave("../output/Molecular/lncRNA/WGCNA/module_boxplot.pdf", plot = last_plot(), height = 15, width = 25)
ggsave("../output/Molecular/lncRNA/WGCNA/module_boxplot.png", plot = last_plot(), height = 15, width = 25)
```

Use this plot to determine the groups