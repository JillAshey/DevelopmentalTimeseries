...
#### We had 31 modules before merging, and 17 modules after merging
...
#write.csv(all_info, "../output/WGCNA/module_gene_trait_correlations.csv")
...
pdf(file = "../output/WGCNA/Module-trait-relationship-heatmap.pdf", height = 11.5, width = 8)
ht=Heatmap(moduleTraitCor, name = "Eigengene", column_title = "Module-Trait Eigengene Correlation", 
        ...
        column_order = 1:8, column_dend_reorder = FALSE, cluster_columns = hclust(dist(t(moduleTraitCor)), method = "average"), column_split = 4, column_dend_height = unit(0.5, "in"),
...
```{r}
MEs <- read.csv("../output/WGCNA/WGCNA_ModuleEigengenes.csv")
...
maternal <- MEsPlotting %>%
  dplyr::select(MEmidnightblue, MEgreen, MElightgreen) %>%
  dplyr::rowwise() %>%
  dplyr::mutate(Maternal_avg = mean(c_across(everything())))

## Group 2: Wave 1 MZT
wave1_mzt <- MEsPlotting %>%
  dplyr::select(MEdarkturquoise, MElightyellow, MEskyblue, MEsaddlebrown, MEred, MEpaleturquoise) %>%
  dplyr::rowwise() %>%
  dplyr::mutate(Wave1_MZT_avg = mean(c_across(everything())))

## Group 3: Wave 2 MZT
wave2_mzt <- MEsPlotting %>%
  dplyr::select(MEdarkorange, MEorange, MEblack, MEcyan, MEdarkgreen) %>%
  dplyr::rowwise() %>%
...
zga1 <- MEsPlotting %>%
  dplyr::select(MEblue) %>%
  dplyr::rename(ZGA1_avg = MEblue)
# zga <- MEsPlotting %>%
#   dplyr::select(MEblue, MEpink, MEmagenta) %>%
#   dplyr::rowwise() %>%
#   dplyr::mutate(ZGA_avg = mean(c_across(everything())))

## Group 5: ZGA2
zga2 <- MEsPlotting %>%
  dplyr::select(MEpink, MEmagenta) %>%
  dplyr::rowwise() %>%
...
#ggsave("../output/WGCNA/MEs_boxplot.pdf", plot = MEplot, height = 15, width = 25)
#ggsave("../output/WGCNA/MEs_boxplot.png", plot = MEplot, height = 15, width = 25)
...
#ggsave("../output/WGCNA/MEs_lineplot.pdf", plot = MEplot_line, height = 15, width = 25)
#ggsave("../output/WGCNA/MEs_lineplot.png", plot = MEplot_line, height = 15, width = 25)
```
...
all_info_group <- all_info %>%
  mutate(Group = case_when(
    ...
    moduleColor %in% c("darkturquoise", "lightyellow", "skyblue", "saddlebrown", "red", "paleturquoise") ~ "Wave1_MZT",
    moduleColor %in% c("darkorange", "orange", "black", "cyan", "darkgreen") ~ "Wave2_MZT",
