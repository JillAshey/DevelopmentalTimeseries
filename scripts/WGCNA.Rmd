...
num_genes <- all_info %>%
  ...
  summarise(num_queries = n_distinct(gene_id))
...
maternal <- MEsPlotting %>%
  select(MEmidnightblue, MEgreen, MElightgreen) %>%
  rowwise() %>%
  mutate(Maternal_avg = mean(c_across(everything())))

## Group 2: Wave 1 MZT
wave1_mzt <- MEsPlotting %>%
  select(MEdarkturquoise, MElightyellow, MEskyblue, MEsaddlebrown, MEred, MEpaleturquoise) %>%
  rowwise() %>%
  mutate(Wave1_MZT_avg = mean(c_across(everything())))

## Group 3: Wave 2 MZT
wave2_mzt <- MEsPlotting %>%
  select(MEdarkorange, MEorange, MEblack, MEcyan, MEdarkgreen) %>%
  rowwise() %>%
  mutate(Wave2_MZT_avg = mean(c_across(everything())))

## Group 4: ZGA1
zga1 <- MEsPlotting %>%
  select(MEblue) %>%
  rename(ZGA1_avg = MEblue)

## Group 5: ZGA2
zga2 <- MEsPlotting %>%
  select(MEpink, MEmagenta) %>%
  rowwise() %>%
  mutate(ZGA2_avg = mean(c_across(everything())))

# Combine all averages
MEsPlotting_avg <- bind_cols(
  MEsPlotting %>% select(TubeID, hpf),
  maternal %>% select(Maternal_avg),
  wave1_mzt %>% select(Wave1_MZT_avg),
  wave2_mzt %>% select(Wave2_MZT_avg),
  zga1,
  zga2 %>% select(ZGA2_avg)
)

MEsPlotting_melt <- melt(MEsPlotting_avg, id = c("TubeID", "hpf"))
