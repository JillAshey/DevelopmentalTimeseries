...
Remove M14 and M61 - these are outliers (based on PCA)
```{r}
...
save(sft, file = "~/Desktop/PutnamLab/DevelopmentalTimeseries/sft.RData")
load("~/Desktop/PutnamLab/DevelopmentalTimeseries/sft.RData")
...
# png to output 
png("../output/Molecular/mRNA/WGCNA/sft.png", 1000, 1000, pointsize=20)
par(mfrow = c(1,2));
cex1 = 0.9;
...
save(TOM_sign, file = "~/Desktop/PutnamLab/DevelopmentalTimeseries/TOM_sign.Rdata")
load("~/Desktop/PutnamLab/DevelopmentalTimeseries/TOM_sign.Rdata")
...
sizeGrWindow(8,6)
png("../output/Molecular/mRNA/WGCNA/GeneDendrogram.png", 1000, 1000, pointsize=20)
plotDendroAndColors(geneTree_sign, dynamicColors_sign, "Dynamic Tree Cut - SIGNED",
                    dendroLabels = FALSE, hang = 0.03,
                    addGuide = TRUE, guideHang = 0.05,
                    main = "Gene dendrogram and module colors 'SIGNED'")
...
# Plot the result
sizeGrWindow(7, 6)
png("../output/Molecular/mRNA/WGCNA/ClusterEigengenes.png", 1000, 1000, pointsize=20)
...
sizeGrWindow(7, 6)
png("../output/Molecular/mRNA/WGCNA/ClusterEigengenes_merged.png", 1000, 1000, pointsize=20)
plot(METree2, main = "Clustering of module eigengenes - SIGNED (dissimilarity calc = MEDiss = 1-cor(MEs))",
     xlab = "", sub = "")
dev.off()
```
...
png("../output/Molecular/mRNA/WGCNA/ClusterDendrogram_signed.png", 1000, 1000, pointsize=20)
plotDendroAndColors(geneTree_sign, cbind(dynamicColors_sign, mergedColors),
                    c("Dynamic Tree Cut", "Merged dynamic"),
                    dendroLabels = FALSE, hang = 0.03,
                    addGuide = TRUE, guideHang = 0.05)
...
save(MEs, moduleLabels, moduleColors, geneTree_sign, file = "~/Desktop/PutnamLab/DevelopmentalTimeseries/networkConstruction-stepByStep.RData")
# write csv - save the module eigengenes
write.csv(MEs, file = "../output/Molecular/mRNA/WGCNA/WGCNA_ModuleEigengenes.csv")
table(mergedColors)
```
...
gene_id <- read.csv("../output/Molecular/mRNA/mRNA_filtered_counts.csv")[,1]
...
write.csv(all_info, "../output/Molecular/mRNA/WGCNA/module_gene_trait_correlations.csv")
...
pdf(file = "../output/Molecular/mRNA/WGCNA/Module-trait-relationship-heatmap.pdf", height = 11.5, width = 8)
...
Plot by module 
```{r}
MEs <- read.csv("../output/Molecular/mRNA/WGCNA/WGCNA_ModuleEigengenes.csv")
...
ggsave("../output/Molecular/mRNA/WGCNA/module_boxplot.pdf", plot = last_plot(), height = 15, width = 25)
ggsave("../output/Molecular/mRNA/WGCNA/module_boxplot.png", plot = last_plot(), height = 15, width = 25)
```

Based on the boxplots, here is how I am grouping each module: 

- Maternal - skyblue, yellow, darkorange, lightcyan, lightgreen, midnightblue
- Minor wave - cyan, darkgrey, darkred, greenyellow, lightyellow, salmon, white, grey60
- Major wave - black, brown, dark green, darkturquoise
...
maternal <- MEsPlotting %>%
  dplyr::select(MEskyblue, MEyellow, MEdarkorange, MElightcyan, MElightgreen, MEmidnightblue) %>%
  dplyr::rowwise() %>%
  dplyr::mutate(Maternal_avg = mean(c_across(everything())))

## Group 2: Minor Wave
minor <- MEsPlotting %>%
  dplyr::select(MEcyan, MEdarkgrey, MEdarkred, MEgreenyellow, MElightyellow, MEsalmon, MEwhite, MEgrey60) %>%
  dplyr::rowwise() %>%
  dplyr::mutate(MinorWave_avg = mean(c_across(everything())))

## Group 3: Major Wave
major <- MEsPlotting %>%
  dplyr::select(MEblack, MEbrown, MEdarkgreen, MEdarkturquoise) %>%
  dplyr::rowwise() %>%
...
ggsave("../output/Molecular/mRNA/WGCNA/groups_boxplot.pdf", plot = MEplot, height = 15, width = 25)
ggsave("../output/Molecular/mRNA/WGCNA/groups_boxplot.png", plot = MEplot, height = 15, width = 25)
...
ggsave("../output/Molecular/mRNA/WGCNA/groups_lineplot.pdf", plot = MEplot_line, height = 15, width = 25)
ggsave("../output/Molecular/mRNA/WGCNA/groups_lineplot.png", plot = MEplot_line, height = 15, width = 25)
```
...
# Save as csv
write.csv(all_info_group, "../output/Molecular/mRNA/WGCNA/module_gene_trait_correlations_groups.csv")
