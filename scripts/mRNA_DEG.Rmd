...
```{r}
# Wald
results_1v4_wald <- as.data.frame(results(boo, name = c("hpf_4.hpf_vs_1.hpf")))
...
# LRT
results_1v4 <- as.data.frame(results(DEG, name = c("hpf_4.hpf_vs_1.hpf")))
results_1v4 <- results_1v4 %>%
  mutate(
    regulation = case_when(
      padj < 0.05 & log2FoldChange < -1 ~ "Up at 1 hpf",
      padj < 0.05 & log2FoldChange > 1 ~ "Up at 4 hpf",
...
# Make a volcano plot
ggplot(data = results_1v4, aes(x = log2FoldChange, y = -log10(padj), color = regulation)) +
  geom_vline(xintercept = c(-1, 1), col = "gray", linetype = 'dashed') +
  geom_hline(yintercept = -log10(0.05), col = "gray", linetype = 'dashed') + 
  geom_point() +
  scale_color_manual(values = c("grey", "#00AFBB", "purple"),
...
write.csv(results_1v14_sig, "../output/Molecular/mRNA/mRNA_DE_results_sig_1v14.csv") 
...
write.csv(results_1v22_sig, "../output/Molecular/mRNA/mRNA_DE_results_sig_1v22.csv") 
...
write.csv(results_1v28_sig, "../output/Molecular/mRNA/mRNA_DE_results_sig_1v28.csv") 
...
write.csv(results_1v48_sig, "../output/Molecular/mRNA/mRNA_DE_results_sig_1v48.csv") 
...
```{r}
# Wald
results_1v72_wald <- as.data.frame(results(boo, name = c("hpf_72.hpf_vs_1.hpf")))
...
## LRT 
results_1v72 <- as.data.frame(results(DEG, name = c("hpf_72.hpf_vs_1.hpf")))
results_1v72 <- results_1v72 %>%
  mutate(
    regulation = case_when(
      padj < 0.05 & log2FoldChange < -1 ~ "Up at 1 hpf",
      padj < 0.05 & log2FoldChange > 1 ~ "Up at 72 hpf",
...
# Subset by padj < 0.05
results_1v72_sig <- results_1v72 %>%
  filter(padj < 0.05 & abs(log2FoldChange) >= 1)
sum(results_1v72_sig$log2FoldChange < 0) # upregulated in 1hpf
sum(results_1v72_sig$log2FoldChange > 0) # upregulated in 72hpf
write.csv(results_1v72_sig, "../output/Molecular/mRNA/mRNA_DE_results_sig_1v72.csv") 

# Make a volcano plot
ggplot(data = results_1v72, aes(x = log2FoldChange, y = -log10(padj), color = regulation)) +
  geom_vline(xintercept = c(-1, 1), col = "gray", linetype = 'dashed') +
  geom_hline(yintercept = -log10(0.05), col = "gray", linetype = 'dashed') + 
  geom_point() +
  scale_color_manual(values = c("grey", "#00AFBB", "purple"),
