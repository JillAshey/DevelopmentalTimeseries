...
p <- ggraph(g_tbl, layout = "fr") +
  ...
  labs(title = "miRNA-mRNA Interaction Network",
       subtitle = "Edge width represents |PCC|, color represents correlation direction");p
ggsave("../output/Molecular/miRNA_mRNA_interactions/maternal_significant_miRNA_mRNA_network_labels.png", p, width = 20, height = 15, dpi = 300)
```
...
Subset minor 
```{r}
minor <- sig_combined_data_pcc_gene_corr_cats %>%
  filter(category == "MinorWave")
length(unique(minor$miRNA))
length(unique(minor$mRNA))
```

Summarize minor wave data
```{r}
summary_data <- minor %>%
  group_by(miRNA) %>%
  summarise(
...
Subset counts matrices for miRNAs and genes in minor wave 
```{r}
## miRNA
# Extract miRNA identifiers from the minor dataset
minor_miRNAs <- minor$miRNA

# Filter mirna_counts_df to keep only rows where the row names are in minor_miRNAs
filtered_mirna_counts <- mirna_counts_df[rownames(mirna_counts_df) %in% minor_miRNAs, ]

## mRNA
# Extract mRNA identifiers from the minor dataset
minor_mRNAs <- minor$mRNA

# Filter df to keep only rows where the row names are in minor_mRNAs
filtered_mrna_counts <- vst_mean_sd_se_df[vst_mean_sd_se_df$Gene %in% minor_mRNAs, ]
```
...
# Save as csv 
write.csv(merged_data_z, "../output/Molecular/miRNA_mRNA_interactions/minor_wave_gene_corr_miranda_PCC_sig_miRNA_mRNA.csv")
```
...
# Save all plots to a PDF
pdf("../output/Molecular/miRNA_mRNA_interactions/minor_wave_miRNA_mRNA_sig_interactions_WGCNA_zscores.pdf", width = 12, height = 8)
for (plot in plot_list) {
  print(plot)
}
dev.off()
...
# Save all plots to a PDF
pdf("../output/Molecular/miRNA_mRNA_interactions/minor_wave_miRNA_mRNA_sig_interactions_WGCNA_counts.pdf", width = 12, height = 8)
for (plot in plot_list) {
  print(plot)
}
dev.off()
...
Make a network plot for genes in minor category targeted by miRNAs
```{r}
# Create the graph
g <- graph_from_data_frame(minor, directed = FALSE)
...
p <- ggraph(g_tbl, layout = "fr") +
  ...
  labs(title = "miRNA-mRNA Interaction Network",
       subtitle = "Edge width represents |PCC|, color represents correlation direction");p
ggsave("../output/Molecular/miRNA_mRNA_interactions/minor_wave_significant_miRNA_mRNA_network.png", p, width = 12, height = 10, dpi = 300)
```

Count number of targets for each miRNA in minor module 
```{r}
# Count the number of target mRNAs for each miRNA
result <- minor %>%
  group_by(miRNA) %>%
...
ggsave("../output/Molecular/miRNA_mRNA_interactions/minor_significant_miRNA_num_targets.png", target_num, width = 12, height = 10, dpi = 300)
ggsave("../output/Molecular/miRNA_mRNA_interactions/minor_significant_miRNA_num_targets.pdf", target_num, width = 12, height = 10, dpi = 300)
```
...
Subset major 
```{r}
major <- sig_combined_data_pcc_gene_corr_cats %>%
  filter(category == "MajorWave")
length(unique(major$miRNA))
length(unique(major$mRNA))
```

Summarize major wave data
```{r}
summary_data <- major %>%
  group_by(miRNA) %>%
  summarise(
...
Subset counts matrices for miRNAs and genes in major wave 
```{r}
## miRNA
# Extract miRNA identifiers from the major dataset
major_miRNAs <- major$miRNA

# Filter mirna_counts_df to keep only rows where the row names are in major_miRNAs
filtered_mirna_counts <- mirna_counts_df[rownames(mirna_counts_df) %in% major_miRNAs, ]

## mRNA
# Extract mRNA identifiers from the major dataset
major_mRNAs <- major$mRNA

# Filter df to keep only rows where the row names are in major_mRNAs
filtered_mrna_counts <- vst_mean_sd_se_df[vst_mean_sd_se_df$Gene %in% major_mRNAs, ]
```
...
# Save as csv 
write.csv(merged_data_z, "../output/Molecular/miRNA_mRNA_interactions/major_wave_gene_corr_miranda_PCC_sig_miRNA_mRNA.csv")
```
...
# Save all plots to a PDF
pdf("../output/Molecular/miRNA_mRNA_interactions/major_wave_miRNA_mRNA_sig_interactions_WGCNA_zscores.pdf", width = 12, height = 8)
for (plot in plot_list) {
  print(plot)
}
dev.off()
...
# Save all plots to a PDF
pdf("../output/Molecular/miRNA_mRNA_interactions/major_wave_miRNA_mRNA_sig_interactions_WGCNA_counts.pdf", width = 12, height = 8)
for (plot in plot_list) {
  print(plot)
}
dev.off()
...
Make a network plot for genes in major category targeted by miRNAs
```{r}
# Create the graph
g <- graph_from_data_frame(major, directed = FALSE)
...
p <- ggraph(g_tbl, layout = "fr") +
  ...
  labs(title = "miRNA-mRNA Interaction Network",
       subtitle = "Edge width represents |PCC|, color represents correlation direction");p
ggsave("../output/Molecular/miRNA_mRNA_interactions/major_wave_significant_miRNA_mRNA_network.png", p, width = 12, height = 10, dpi = 300)
```

Count number of targets for each miRNA in major module 
```{r}
# Count the number of target mRNAs for each miRNA
result <- major %>%
  group_by(miRNA) %>%
...
ggsave("../output/Molecular/miRNA_mRNA_interactions/major_significant_miRNA_num_targets.png", target_num, width = 12, height = 10, dpi = 300)
ggsave("../output/Molecular/miRNA_mRNA_interactions/major_significant_miRNA_num_targets.pdf", target_num, width = 12, height = 10, dpi = 300)
```



I think I will need to edit how I am doing the expression over time plots - decide whether to normalize the counts, log them, zscore, etc 
