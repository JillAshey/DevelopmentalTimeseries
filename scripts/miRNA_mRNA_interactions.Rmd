...
mirna_counts_df <- mirna_counts_df %>%
  group_by(X.miRNA) %>%
  summarise(across(ends_with(".norm."), mean))
...
# Save as csv
write.csv(combined_data_pcc, "../output/Molecular/miRNA_mRNA_interactions/Peve-miranda_PCC_miRNA_mRNA.csv")
```
...
plot_data <- merged_data_z %>%
  select(miRNA, mRNA, timepoint, mirna_zscore, mrna_mean_zscore, mrna_se_zscore) %>%
...
# Save all plots to a PNG
png("../output/Molecular/miRNA_mRNA_interactions/maternal_miRNA_mRNA_sig_interactions_WGCNA_zscores.png", width = 12, height = 8)
for (plot in plot_list) {
  print(plot)
}
dev.off()
