...
```{r}
specific_mirna <- "Montipora_capitata_HIv3___Scaffold_1_4183"

interactions <- miranda %>%
  filter(miRNA == specific_mirna) %>%
  select(miRNA, mRNA) #%>%
  #slice(100:150)  # Adjust this number as needed
```
...
```{r}
plot_data <- bind_rows(
  mirna_data %>% rename(id = mirna),
  mrna_data %>% rename(id = gene)
) %>%
...
# Step 5: Generate and display plots for each pair
