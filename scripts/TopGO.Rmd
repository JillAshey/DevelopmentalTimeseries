...
test <- annot %>%
  ...
  dplyr::filter(V1 == "Montipora_capitata_HIv3___Scaffold_14_1018693") #%>%
...
network_plot <- ggraph(g_tbl, layout = "fr") +
  geom_edge_link(aes(edge_width = weight, color = color), alpha = 0.7) +
  geom_node_point(aes(color = type), size = 15) +
  geom_node_text(aes(label = name), repel = TRUE, size = 10, 
                 family = "Arial", fontface = "bold") +
  scale_edge_width(range = c(2, 6), name = "Correlation Strength") +
...
ggsave("../output/Molecular/miRNA_mRNA_interactions/GO_en_sig_miRNA_targets_network_Montipora_capitata_HIv3___Scaffold_14_1018693.png", plot = network_plot, height = 20, width = 15)
```
...
pdf("../output/Molecular/miRNA_mRNA_interactions/all_mirna_plots.pdf", width = 10, height = 8)
